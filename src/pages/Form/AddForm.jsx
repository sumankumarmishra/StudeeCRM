...
const AddForm = () => {
  ...
  const handleSubmitForm = async (e) => {
    e.preventDefault();
    const { formUrl, filename } = await generateFormFile();
    const data = {
    ...
    if (isAllRequiredFieldsPresent) {
      axios
        .post("https://crm.internationaleducationoffice.co.uk/forms", data, config)
        .then((response) => {
          setPopupshow(true);
          setPopupText("Form Added");
          setTimeout(() => {
            setPopupshow(false);
  ...
  const generateFormFile = async () => {
    const formContent = generateFormContent();

    try {
      const response = await axios.post(
        "https://crm.internationaleducationoffice.co.uk/generate-form",
        {
          formContent,
        },
        config
      ...
      return { formUrl: formUrl, filename: filename };
    } catch (error) {
      console.error("Error generating form file:", error);
        ...
        <Container fluid className="p-5">
          <div>
            ...
            <Button
              ...
              className="submit-button mt-2 py-2 px-4"
            >
              Submit
            </Button>
          </div>
