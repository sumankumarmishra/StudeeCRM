...
const DatatableRatings = () => {
  ...
  const handleApprove = (id) => {
    axios
      .put("https://crm.internationaleducationoffice.co.uk/ratings/" + id, { approve: true }, config)
      .then((response) => {
        console.log(response.data);
      });

    setPopupshow(true);
    setPopupText("Rating Approved");
    setTimeout(() => {
      setPopupshow(false);
  ...
  const handleUnpprove = (id) => {
    axios
      .put("https://crm.internationaleducationoffice.co.uk/ratings/" + id, { approve: false }, config)
      .then((response) => {
        console.log(response.data);
      });

    setPopupshow(true);
    setPopupText("Rating Unapproved");
    setTimeout(() => {
      setPopupshow(false);
      window.location.reload();
    }, 800);
        ...
        return (
          <div className="cellAction">
            ...
            <div
              ...
              onClick={() => handleUnpprove(params.row._id)}
            >
              UnApprove
            </div>
          </div>
  ...
  return (
    <div className="datatable">
      <div className="datatableTitle">Ratings</div>
      ...
      {popUpShow ? (
        <PopupAlert
          popUpText={popUpText}
          backgroundColor={popUpText === "Rating Approved" ? "#8AFF8A" : "red"}
        />
      ) : (
        ""
      )}
...
export default DatatableRatings;
